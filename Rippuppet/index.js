...
const adminCookieName = config.values.adminCookieName;
const adminCookieValue = config.values.adminCookieValue;
...
console.log(inputValues);
  ...
  (async () => {
    ...
    for(b of browsers){
      ...
      // inserta cookies para entrar a Ghost
      await context.addCookies([{name: adminCookieName, value: adminCookieValue, path: "/", domain: "localhost"}]);
...
async function fillInput(elementHandle, page){
  ...
  if(type === 'text'){
    ...
    elementHandle.click();
    page.keyboard.type(faker.string.alpha());
  }
